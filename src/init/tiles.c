...
static void	set_values(
	t_tsr_tile_data *tile, t_mlem_value objects, t_mlem_value object)
{
	t_mlem_value	*value;
	...
	tile->light = get_as_vec3(object, "light", vec3_zero());
	...
	value = mlem_object_get(object, "interact_to");
	if (value)
		tile->interact_to = mlem_object_key_index(objects,
				value->refv.value->name);
	value = mlem_object_get(object, "timeout_to");
	if (value)
		tile->timeout_to = mlem_object_key_index(objects,
				value->refv.value->name);
	tile->timeout_time = mlem_as_float(
			mlem_object_get(object, "timeout_time"), 0);
}

static t_tsr_tile_data	load_tile(
	t_tsr *tsr, t_mlem_value objects, t_mlem_value object, t_mlem_string name)
{
	const t_tsr_tile_data	def = {
		.interact_to = -1, .timeout_to = -1, .texture = {
	...
	set_values(&tile, objects, object);
...
void	load_tile_data(t_tsr *tsr)
{
	...
	tiles = mlem_parse(TSR_ROOT "data/tiles.mlem", NULL, tsr->textures.mlem);
	if (!tiles.type)
	...
	while (i < tiles.objectv.len && i < TILE_BUFFER_SIZE)
	{
		...
		tsr->world_data.tiles[i] = load_tile(
				tsr, tiles, tile->value, tile->name);
